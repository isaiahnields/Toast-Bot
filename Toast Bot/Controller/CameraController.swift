...
extension CameraController {
    func prepare(completionHandler: @escaping (Error?) -> Void) {
        ...
        func configureCaptureDevices() throws {
            ...
            let session = AVCaptureDevice.DiscoverySession(deviceTypes: [.builtInDualCamera, .builtInTrueDepthCamera], mediaType: AVMediaType.video, position: .unspecified)
    ...
    public func photoOutput(_ captureOutput: AVCapturePhotoOutput, didFinishProcessingPhoto photoSampleBuffer: CMSampleBuffer?, previewPhoto previewPhotoSampleBuffer: CMSampleBuffer?,
        ...
        if let error = error { self.photoCaptureCompletionBlock?(nil, error) }
            ...
            let flippedImage = ciImage.transformed(by: CGAffineTransform(scaleX: -1.0, y: 1.0))
                self.photoCaptureCompletionBlock?(UIImage.convert(from: flippedImage), nil)
            }
